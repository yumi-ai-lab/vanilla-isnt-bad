...
// sky composite loads independently; failures leave the complete photograph.
// App descriptions, mockups and launch links never wait for image decoding.
for (const [image,setReady] of [
  [cupImage,value=>cupReady=value],
  [landscape,value=>landscapeReady=value],
  ...atmosphereImages.map(image=>[image,value=>value ? decodedAtmosphere.add(image) : decodedAtmosphere.delete(image)])
]) {
  let retries = 0;
  observeImage(image, {
