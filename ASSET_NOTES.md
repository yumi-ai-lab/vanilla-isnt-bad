...
空と変化する雲のcanvas、前景のアルファで抜いた採用写真、カップの順に重ねています。元写真の静止した雲と二重に重ねません。前景は低優先度で読み込み、デコードと初期描画に成功した時だけ650msで空を切り替えます。追加素材の失敗時やWebGLが使えない時は元の写真を表示。画面外・タブ非表示では描画予約を止め、動き軽減時は雲を静止します。木々のぼけとカップの光は今回変更していません。
