# 空と公園、中央のカップと変化する雲（現在の詳細画面）

2026-09-19 配信用形式を軽量化。新しい絵は生成せず、元PNGを保持したまま1536×1024でWebPへエンコードしました。アイスは可逆圧縮で、透明度と見えるRGBの一致を全画素で確認。前景は未使用のRGBを白にして元のalphaだけを可逆保存し、alphaも全画素一致。背景は品質96で、RGBのPSNRは約44.59dB、ブラウザーでも見え方を確認しました。構図・色補正・ぼけ・カップの形や影は同じです。

| 配信用ファイル | bytes | 原本PNG bytes |
| --- | ---: | ---: |
| terrace-open-park-v8.webp | 330,100 | 2,130,511 |
| flavor-cutouts-v2.webp | 869,846 | 1,981,985 |
| flavor-scoops-top-v2.webp | 1,403,178 | 2,605,891 |
| terrace-park-mask-v1.webp | 179,548 | 2,298,869 |
| 合計 | 2,782,672 | 9,017,256 |

約69.1%削減。再現スクリプトは scripts/encode-menu-assets.mjs。スマホのメニューでは一覧用だけ、詳細へ直接進む場合は背景・カップだけを先に取得し、空のマスクは商品のデコード後に取得します。以下のPNG容量や生成記録は原本の履歴です。

2026-09-18 奥行きと雲の仕上げ。画像ファイル・構図・既存の色補正は保持しています。遠い公園の木々へ、同じ背景写真と `blur(clamp(.75px,.30cqw,3px))` を楕円のマスク（中心73%・47%、半径50%・23%）で重ねます。空の合成では前景アルファの内側に置くため、元写真の静止雲は戻りません。静止画の代替表示にも同じ調整を適用。雲は座標の縦横比を調整し、濃度場を5段階、奥行きのサンプルを28点として薄い縁と内部の陰影を調整しました。描画上限420px・12fpsと停止処理は維持。カップの画像・大きさ・光・影は変更していません。

2026-09-18 色調整。画像ファイルは保持し、`menu-demo.css` で背景の彩度を79%、コントラストを95%、明るさを101.8%に調整しました。天板は画面高67〜76%の範囲でなだらかに有効になる `#eee9dd`・38%・color合成で青紫を抑えます。補正層はカップと接地影より後ろに置き、構図・輪郭・表面の質感を保ちます。新たな画像生成や画像ファイルの加工はありません。

2026-09-18 更新。雲のPNG反復を cloud-sky.js の独自WebGL実装に変更しました。[webgl - cloudy / strangerintheq](https://codepen.io/strangerintheq/pen/OKVVOW) の体積濃度の考え方を参考にしたもので、ソースの転載・外部埋め込み・外部ライブラリはありません。32³の疑似乱数場をパディング付きテクスチャとしてコード内で作り、空と雲を一緒に描きます。新たな画像加工や生成は行っていません。下記の透過雲PNGとそのプロンプトは制作履歴としてのみ保持しています。
...
- `assets/terrace-clouds-v1.png` — 1536×1024、1,147,566 bytes、RGBA。以前の透過雲素材。青灰色の半透明な縁と画像の反復が気になったため、現在のページでは読み込みません。
...
空と変化する雲のcanvas、前景のアルファで抜いた採用写真、カップの順に重ねています。元写真の静止した雲と二重に重ねません。前景は低優先度で読み込み、デコードと初期描画に成功した時だけ650msで空を切り替えます。追加素材の失敗時やWebGLが使えない時は元の写真を表示。画面外・タブ非表示では描画予約を止め、動き軽減時は雲を静止します。遠い木々にのみ追加のぼかしを重ね、カップの光は保っています。
