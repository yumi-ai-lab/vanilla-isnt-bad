...
- 押す反応120ms、状態変化240ms、初回の登場420msを基準に、減速を `cubic-bezier(.22,.61,.36,1)` で揃えています。看板は最初に見えたときだけ420ms、最大1.5度の小さな揺れ。
- 通常の縦スクロール。ナビと冒頭のリンクは中央から下線が伸び、冒頭の矢印は4px下がります。キーボードのフォーカスでも同じ反応を返します。
- アプリのアイコンはホバーとフォーカスで2px、選択時に4px持ち上がり、押すと98%に縮みます。実際のアプリがあるケースだけ内側の光が少し強くなります。ラベルはすぐに読めます。
