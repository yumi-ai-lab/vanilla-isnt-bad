...
詳細はカップを手元に置いた商品写真の構図です。カップは画像の高さの約57%を占め、背景の左端には近くのお店の一部をぼかして残します。テーブルの脚と手前の縁を画角から外し、カップの底に潜り込む接地楕円と、天板へ広く薄く減衰する二層の影で支えます。影はグラデーションで描き、カップ本体の画像はぼかしません。底の位置は透過PNGのほぼ不透明な部分に合わせ、上段は13.1%、下段は14.45%。動きなしでも影の濃さは変わりません。皿や配膳は加えていません。

背景 assets/terrace-open-view-v4.png は、お店の脇でアイス越しに景色を見る構図です。天板を画面下の約3分の1へ抑え、低めの視点から、左端の近くの店、右端の少し離れた白いベンチ、淡い舗装と奥の景色を見渡せるようにしました。緑は周辺に留め、柔らかな木漏れ日は奥の舗装だけに置いています。カップはサイズを保ち、従来から画像高の4.5%ぶん下へ移動しました。天板とカップには動く葉影や光の溜まりを重ねず、接地影はカップと一緒に移動します。

風は背景写真の上端にある二箇所の葉だけで表します。同じ写真を同じ縮尺で再利用し、固定した柔らかいCSSマスク内で、ごく小さな位置の変化を加えます。移動量は1〜2px、周期は左18秒・右15.5秒。新しい葉の位置にマスクを合わせ、店・ベンチ・天板・カップは動かしません。背景は内蔵画像生成で編集しました。風のための別画像や追加ダウンロードは不要です。
