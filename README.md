...
詳細はカップを手元に置いた商品写真の構図です。カップは画像の高さの約57%を占め、背景の左上には近くのお店のひさしを残します。テーブルの脚と手前の縁を画角から外し、カップの底に潜り込む接地楕円と、天板へ広く薄く減衰する二層の影で支えます。影はグラデーションで描き、カップ本体の画像はぼかしません。底の位置は透過PNGのほぼ不透明な部分に合わせ、上段は13.1%、下段は14.45%。動きなしでも影の濃さは変わりません。皿や配膳は加えていません。

背景 assets/terrace-courtyard-material-v6.png は、ユーザーが選んだ白い中庭を基に、ベンチの斜め方向と座面の奥行き、白い金属天板の細かな塗装肌と柔らかい反射を整えた画像です。空と低い壁、左上の淡い黄色のひさしを残し、天板は画面下の約27%。カップのサイズ・位置・接地影は維持し、天板とカップには動く葉影や光の溜まりを重ねません。

風は独立した透過PNG assets/terrace-branch-v1.png で右上の枝に加えます。背景から同じ枝を取り除いてあるので、葉の隙間には空が見え、静止した葉と二重になりません。画面右上の枝元を軸に9秒周期で−1.8〜2.4度回転し、わずかな移動を加えます。スマホでは枝先が数px揺れる程度。遠景の木・ひさし・壁・ベンチ・天板・カップは静止します。背景は約1.85MB、追加の透過PNGは約0.98MBです。

動きは画面外・非表示タブでは一時停止し、OSまたはサイトの「動きなし」では枝を自然な角度で静止させます。カップ・背景・枝のデコード完了後に葉を表示します。枝の読み込みが失敗しても、カップと説明の表示は妨げません。外部サービスの埋め込みや動画、毎フレームのJavaScript処理はありません。
...
- 選んだカップは、白い金属テーブルの手元に大きく置きます。背景は白い中庭と空、左上のひさし。独立した右上の枝にコードで穏やかな揺れを加えます。PCはメニューと説明を横並びにし、900px以下はメニューから説明へ切り替えます。
