...
背景 assets/terrace-airy-courtyard-v5.png は、ユーザーが選んだ白い中庭の画像です。空と低い壁、左奥の白いベンチ、左上の淡い黄色のひさしで、お店の脇の休憩スペースを表します。緑は左右の端に留め、天板は画面下の約27%。カップのサイズと位置、接地影は維持しています。天板とカップには動く葉影や光の溜まりを重ねません。

風は左端の木の葉と右上の葉だけで表します。同じ写真を同じ縮尺で再利用し、固定した柔らかいCSSマスク内で、ごく小さな位置の変化を加えます。移動量は1〜2px、周期は左18秒・右15.5秒。ひさし・壁・ベンチ・天板・カップは動かしません。指定画像を加工せず採用し、風のための別画像や追加ダウンロードは不要です。
...
- 選んだカップは、白い金属テーブルの手元に大きく置きます。背景は白い中庭と空、左上のひさし。左右の葉先だけにコードで微量な揺れを加えます。PCはメニューと説明を横並びにし、900px以下はメニューから説明へ切り替えます。
