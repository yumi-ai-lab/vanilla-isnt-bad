...
背景 `assets/terrace-foreground-v2.png` に強い木漏れ日は焼き込んでいません。天板の曲線に沿う固定マスクの中で、奥・中・手前の葉影を34秒・26秒・21秒で動かします。各深度帯のマスクと濃度は固定し、SVGを横長に縮めて天板上の遠近を表します。奥二層は横に繰り返して配置します。

斜めに細長い五つの光の溜まりは、同じ天板マスクの中で screen 合成します。手前の葉影と同じ21秒・同じ座標の一枚の層として動かし、位置と回転のずれを抑えます。既存の広い明部は不透明度16%の弱い補助光です。カップにはPNGの固定マスク内だけに薄い葉影を重ね、手前の影と座標・周期を合わせます。カップ本体と接地影の不透明度は変えません。

動きは画面外・非表示タブでは一時停止し、OSまたはサイトの「動きなし」では葉影と光の溜まりを静止させます。画像の準備ができるまでは光の層も表示しません。外部サービスの埋め込みや動画、毎フレームのJavaScript処理はありません。

一覧の用途は13px・濃いグレーで、名前の下から読み始められるようにしています。正方形の画像と2〜3列の配置は維持します。

一覧の画像を先読みし、詳細のカップもメニュー閲覧中に読み込み始めます。カップはCSS背景ではなく、実際に表示する一枚のimgを切り抜いて使います。カップと背景の両方のデコードを待って接地影・木漏れ日と一緒に表示し、読み込み失敗時は画像ごとに一度だけ再試行します。切替時のフェードもカップと接地影を一緒に扱います。
...
- 選んだカップは、白い金属テーブルの手元に大きく置きます。奥のお店はぼかし、木漏れ日の揺れをコードで重ねます。PCはメニューと説明を横並びにし、900px以下はメニューから説明へ切り替えます。
...
- assets/terrace-canopy-v2.svg — コードで作成した葉影。v1の図形を保持し、preserveAspectRatio="none" で縦横の別倍率を許可。天板とカップの固定マスク内で使います。v1は以前の表示との比較用に保持します。
