...
- 「すべてのアプリ」とAPPS看板から apps.html の独立したショーケースへ進みます。PCは3列、680px以下は2列。アプリが増えると一覧の行だけが増え、店舗画像は伸びません。
- ショーケースは一つの白い筐体、共通の銀の棚板、内側のガラス札。メインと同じ日よけと淡い公園をつなぎ、六種類のカップを少し小さく並べています。透過PNGの奥行きとHTMLの名前・用途を重ね、ホバーとフォーカスで内側の光がわずかに変わります。
- アプリを選ぶと、用途・できること・画面イメージを詳細ダイアログで表示。Escape、閉じるボタン、外側のクリック、ブラウザーの戻るで閉じ、選んだアプリへフォーカスとスクロール位置を戻します。
...
- 冒頭の浮上とぼけ、窓の開閉、空からクリームへの変化、ケースの薄い冷気を維持。ショーケースの登場は360ms。端末とサイトの動きを減らす設定を尊重します。
...
店頭は最大三件、別画面は全件を登録順で表示します。棚の行数は件数に合わせて増え、最後の行に空きがあっても横の棚板が途切れません。DOMの検証では、確認用の12件でも店頭が増えず、全件に個別の詳細があることを確認しています。検索・分類・ページ分割は未実装です。実際の種類や件数が増えた時点で一覧側へ追加できます。
...
自動テストは、実データの正規化、URLの安全性、三つの重複しない選出、サンプルの起動リンク不在、画面画像、日英の原稿、静的配信、共有コードのキャッシュ更新を検証します。既存のプレビュー配信のテストも維持しています。
...
- assets/flavor-cutouts-v2.png — 六つのカップを3列×2行で並べた透過PNG。CSSの背景位置で一つずつ表示し、共通の棚に直接置いています。旧 flavor-collection-v1.png から内蔵 image_gen で背景だけを透過し、元画像も保持しています。
- 日よけは storefront-blank-placards-v6.png の一部をCSS背景で表示。周囲には既存の park-atmosphere.png を薄く表示しています。画像ファイル自体の切り抜き加工は行っていません。
