...
背景 `assets/terrace-foreground-v2.png` に強い木漏れ日は焼き込んでいません。コードで作成した葉影のSVGとCSSの26秒・31秒のアニメーションを、カップとテーブルに共通の層として重ねます。動きは画面外・非表示タブでは一時停止し、OSまたはサイトの「動きなし」では薄い静止影になります。外部サービスの埋め込みや動画、毎フレームのJavaScript処理はありません。
...
- assets/terrace-canopy-v1.svg — コードで作成した柔らかい葉影。CSSで位置と濃さを動かし、カップと天板を共通の光で包みます。
