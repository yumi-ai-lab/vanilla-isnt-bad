...
- 冒頭は両言語で「Good as it is.」を大きく、その下に「Yours if you want.」を16〜18pxで添えます。日本語版だけ「足さなくてもいい。」「だけど、足してもいい。」を14〜16pxの2行で添え、受け手が意味を拾える余白を残します。説明文は15〜16px。ヘッダーと棚・会社説明・フッターの左右の基準を揃えています。
- 中段の棚の見出しは両言語で「APPS」。欧文として同じ文字組みで表示します。
...
下の棚は同じ kiosk-warm.png をCSSの背景にも使います。1309×1201pxの元画像から、中央のケースを含む(495, 726)起点の320×320pxを正方形の枠内に表示しています。画像ファイル自体は変更しておらず、追加のケース画像も配信しません。元画像を変更する場合は styles.css の .case-frame に記載した表示範囲も確認してください。将来のアプリアイコンは .glass-pane の中に独立して載ります。
