...
- ショーケースは白い区画、細い縁、透明感のある札。六種類のアイスの画像に名前・用途をHTMLで添えています。区画へのホバーとフォーカスで内側の光がわずかに変わります。
...
店頭は最大三件、別画面は全件を登録順で表示します。DOMの検証では、確認用の12件でも店頭が増えず、全件に個別の詳細があることを確認しています。検索・分類・ページ分割は未実装です。実際の種類や件数が増えた時点で一覧側へ追加できます。
...
- assets/flavor-collection-v1.png — 六つのカップを3列×2行で生成した画像。CSSの背景位置で一つずつ表示し、元PNGは加工していません。
