...
`menu-demo.html?lang=ja` は、メニューから探す流れを確認する独立したデモです。既存のトップページ・apps.html は変更していません。皿と配膳を使わず、180msの短いフェードで選択を返します。PCは一覧と説明を左右に、900px以下は一覧から説明へ切り替え、戻った際に検索・分類・追加件数・スクロール位置を保ちます。

名前・用途・説明に加え、デモの仕様でも検索できます。説明は3つまでの文字付きミニアイコン、起動ボタンの配置、一文のこだわりと任意の続き。`demo-content.js` の仕様・こだわりはすべて仮のサンプルです。このデモでは起動ボタンを無効化し、実アプリへは接続しません。実アプリのデータにサンプルのこだわりを流用しません。
...
- 最初の選択だけ、カップを薄い白いトレーに載せて約1.2秒で届けます。続けて選ぶ際はトレーを動かさず、480msでカップを入れ替えます。名前と説明、次の操作は演出の完了を待ちません。トレーはCSSで描画し、追加画像のダウンロードは不要です。動き軽減では静止状態にし、別操作やタブ非表示で途中の演出を解除します。
