...
雲は独立した透過PNG assets/terrace-clouds-v1.png を二枚つなぎ、一画面分を240秒かけて横へ流します。同じ画像を繰り返して使うため、周期の境目でも位置は連続します。空を抜いた assets/terrace-park-foreground-v1.png はアルファマスクとしてのみ使い、公園・店・天板の色やぼけは採用写真の画素を保ちます。雲は枝や葉の後ろを通り、元写真の静止した雲は空の層で置き換えます。木・店・天板・カップは静止します。写真は約2.13MB、マスクは約2.30MB、雲は約1.15MBで、追加素材は低優先度の遅延読み込みです。

動きは画面外・非表示タブでは一時停止し、OSまたはサイトの「動きなし」では雲を静止表示します。カップ・背景・空の合成素材のデコード完了後に、650msで空を切り替えます。追加素材が失敗しても、元の完成写真とカップ、説明を表示します。外部サービスの埋め込みや動画、毎フレームのJavaScript処理はありません。
